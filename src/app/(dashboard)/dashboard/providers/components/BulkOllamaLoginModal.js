...
/**
 * BulkOllamaLoginModal — UI untuk auto-add Ollama accounts via Camoufox sidecar service.
 *
 * Flow:
 *   1. User paste email:password per line (textarea)
 *   2. Optional: headless toggle, proxy pool, delay
 *   3. Submit → POST /api/addon/ollama-bulk (Next API route forward ke sidecar)
...
export default function BulkOllamaLoginModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    let res;
    try {
      res = await fetch("/api/addon/ollama-bulk", {
        method: "POST",
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Bulk Auto-Add Ollama Accounts"
      size="lg"
    >
...
BulkOllamaLoginModal.propTypes = {
