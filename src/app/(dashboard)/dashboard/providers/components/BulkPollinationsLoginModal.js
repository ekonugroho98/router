...
/**
 * BulkPollinationsLoginModal — UI untuk auto-add Pollinations accounts via Camoufox sidecar service.
 *
 * Flow:
 *   1. User paste email:password per line (textarea)
 *   2. Optional: headless toggle, proxy pool, delay
 *   3. Submit → POST /api/addon/pollinations-bulk (Next API route forward ke sidecar)
...
export default function BulkPollinationsLoginModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    // Check sidecar
    fetch("/api/addon/pollinations-bulk")
      .then((r) => r.json())
  ...
  const handleSubmit = async () => {
    ...
    let res;
    try {
      res = await fetch("/api/addon/pollinations-bulk", {
        method: "POST",
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Bulk Auto-Add Pollinations Accounts"
      size="lg"
    >
...
BulkPollinationsLoginModal.propTypes = {
