...
/**
 * BulkKiroLoginModal — UI untuk auto-add Kiro accounts via Camoufox sidecar service.
 *
 * Flow:
 *   1. User paste email:password per line (textarea)
 *   2. Optional: headless toggle, proxy pool, delay
 *   3. Submit → POST /api/addon/kiro-bulk (Next API route forward ke sidecar)
...
export default function BulkKiroLoginModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    let res;
    try {
      res = await fetch("/api/addon/kiro-bulk", {
        method: "POST",
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Bulk Auto-Add Kiro Accounts"
      size="lg"
    >
