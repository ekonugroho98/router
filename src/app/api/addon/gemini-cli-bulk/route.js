/**
 * Addon: Gemini CLI Bulk Auto-Add
 *
 * Proxy endpoint dari 9router UI ke unified sidecar service (default port 9100).
 * Sidecar handle multi-provider (Kiro + OpenRouter + Gemini CLI) lewat param `provider`.
 *
 * Flow per akun (handled by sidecar + gemini_login.py):
 *   1. Sidecar spawns gemini_login.py with --email --password --router-url --cli-token
 *   2. gemini_login.py calls GET /api/oauth/gemini-cli/authorize → state + codeVerifier + authUrl
 *   3. Camoufox login at authUrl, captures callback URL with ?code=...
 *   4. Outputs DONE with { code, state, code_verifier, redirect_uri, email }
 *   5. Sidecar calls POST /api/oauth/gemini-cli/exchange → saves connection + projectId
 *
 * Sidecar source: ./addon-kiro-bulk/ (multi-provider, folder name kept buat backward compat).
 */
...
// ─── GET /api/addon/gemini-cli-bulk → cek sidecar health ──────────────────
...
// ─── POST /api/addon/gemini-cli-bulk → forward ke sidecar dengan provider=gemini-cli ───
export async function POST(request) {
  ...
  // Forward ke sidecar dengan provider="gemini-cli"
  let sidecarRes;
  try {
    sidecarRes = await fetch(`${SIDECAR_URL}/bulk-login`, {
      ...
      body: JSON.stringify({
        provider: "gemini-cli",  // ← KEY DIFFERENCE dari kiro-bulk / openrouter-bulk
        accounts,
