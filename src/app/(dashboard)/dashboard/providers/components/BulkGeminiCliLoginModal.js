...
/**
 * BulkGeminiCliLoginModal — UI untuk auto-add Gemini CLI accounts via Camoufox sidecar.
 *
 * Flow:
 *   1. User paste email:password per line (textarea)
 *   2. Optional: headless toggle, proxy pool, delay, max parallel
 *   3. Submit → POST /api/addon/gemini-cli-bulk (Next API route forward ke sidecar)
 *   4. Sidecar per akun:
 *      - Call /api/oauth/gemini-cli/authorize → dapet state + codeVerifier
 *      - Camoufox login Google → capture callback URL
 *      - Call /api/oauth/gemini-cli/exchange → save connection + projectId
 *   5. Receive SSE stream → render real-time progress per account
 *   6. Done → refresh providers list
 *
 * Komponen ini self-contained (cloned dari BulkOpenRouterLoginModal).
 */
...
export default function BulkGeminiCliLoginModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/addon/gemini-cli-bulk")
      .then((r) => r.json())
  ...
  const handleSubmit = async () => {
    ...
    let res;
    try {
      res = await fetch("/api/addon/gemini-cli-bulk", {
        method: "POST",
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Bulk Auto-Add Gemini CLI Accounts"
      size="lg"
    >
      <div className="space-y-4">
        ...
        {/* Gemini CLI usage warning */}
        <div className="rounded-md bg-yellow-500/10 border border-yellow-500/30 p-3 text-xs text-yellow-300">
          ⚠ <strong>Gemini CLI accounts</strong> are designed for use with Gemini CLI tooling
          (Hermes, gemini-cli). Using these accounts with other clients (OpenClaw, Claude, Codex)
          may trigger account restrictions or bans from Google.
        </div>
              ...
              {saveFail.length > 0 && (
                <details className="mt-2" open>
                  <summary className="cursor-pointer text-xs text-yellow-400 font-medium">
                    Save failures ({saveFail.length}) — login berhasil tapi /exchange failed
                    <span className="text-text-muted ml-1">
                      (kemungkinan loadCodeAssist 403 — akun ga punya akses Gemini Code Assist)
                    </span>
                  </summary>
...
BulkGeminiCliLoginModal.propTypes = {
...
function EventLine({ evt }) {
  ...
  return <div className={tone}>{msg}</div>;
}
