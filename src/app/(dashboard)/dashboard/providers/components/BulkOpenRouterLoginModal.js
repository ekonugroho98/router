...
/**
 * BulkOpenRouterLoginModal — UI untuk auto-add OpenRouter accounts via Camoufox sidecar service.
 *
 * Flow:
 *   1. User paste email:password per line (textarea)
 *   2. Optional: headless toggle, proxy pool, delay
 *   3. Submit → POST /api/addon/openrouter-bulk (Next API route forward ke sidecar)
...
export default function BulkOpenRouterLoginModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    // Check sidecar
    fetch("/api/addon/openrouter-bulk")
      .then((r) => r.json())
  ...
  const handleSubmit = async () => {
    ...
    let res;
    try {
      res = await fetch("/api/addon/openrouter-bulk", {
        method: "POST",
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Bulk Auto-Add OpenRouter Accounts"
      size="lg"
    >
...
BulkOpenRouterLoginModal.propTypes = {
