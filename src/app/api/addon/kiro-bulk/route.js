/**
 * Addon: Kiro Bulk Auto-Add
 *
 * Proxy endpoint dari 9router UI ke kiro-bulk sidecar service (default port 9100).
 *
 * Path ini sengaja di `/api/addon/` (folder baru) supaya gak conflict sama
 * route upstream apapun pas sync fork dari decolua/9router.
 *
 * Sidecar source: ./addon-kiro-bulk/ di root repo.
 * Setup sidecar: bash addon-kiro-bulk/install.sh && python addon-kiro-bulk/server.py
 */
...
// ─── GET /api/addon/kiro-bulk → cek sidecar health ──────────────────────────
...
// ─── POST /api/addon/kiro-bulk → forward ke sidecar, stream SSE back ────────
export async function POST(request) {
  ...
  try {
    sidecarRes = await fetch(`${SIDECAR_URL}/bulk-login`, {
      ...
      body: JSON.stringify({
        ...
        // Parallel concurrency (1-5). 1 = sequential (default, safest).
        max_concurrent: Math.max(1, Math.min(5, Number(body?.max_concurrent) || 1)),
        save_to_router: body?.save_to_router !== false, // default true
  ...
  // Stream SSE response back ke client (sidecar response body langsung di-forward)
